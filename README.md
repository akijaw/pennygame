...
- 홈에서 **입장하기** → 학번 4자리 입력 → **참가비 결제**(기본 500코인) → 결제 승인 후 바로 AI 대결로 입장. (1대1 모드는 제거됨)
- 결제는 전부 서버(`/api/coin/*`)가 중개하며, 나플 코인 **API 키는 브라우저에 절대 노출되지 않습니다.**
- AI 대결에서 **승리하면 보상 코인**을 지급합니다(기본 700코인).
  - 승패는 아직 브라우저에서 계산되므로, 보상 실지급은 **기본 비활성화**(`ALLOW_REAL_REWARDS=false`)입니다.
  - 결제 1건(게임 토큰 jti)당 보상은 **최대 1회**만 지급되도록 서버에서 막습니다.
- **"카드 순서 보기"** 버튼으로 코인을 더 내면, 남은 카드 전체 순서를 **본인 화면에만 5초간(게임 중엔 8초)** 보여주고 패턴을 자유롭게 바꿀 수 있습니다.
  - 패턴 선택 화면뿐 아니라 **게임 중(카드 넘기는 도중)에도** 사용할 수 있고, 게임 중엔 팝업 안에서 바로 패턴을 바꿀 수 있습니다.
...
2. Environment Variables에 `.env.example`의 값을 등록:
   ...
   - `ENTRY_COIN_PRICE` = `500`, `WIN_REWARD_COINS` = `700`, `PEEK_COIN_PRICE_BASE` = `30`
