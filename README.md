...
- AI 대결(pve)에서 **승리하면 보상 코인**을 지급합니다(기본 250코인).
  - **1대1(pvp)** 모드에서도 코인 계정 쪽이 이기면 동일하게 보상을 지급합니다 — 모드 선택 시 "코인 계정이 1P/2P 중 누구인지" 물어봅니다.
  - 승패는 아직 브라우저에서 계산되므로, 보상 실지급은 **기본 비활성화**(`ALLOW_REAL_REWARDS=false`)입니다.
  - 결제 1건(게임 토큰 jti)당 보상은 **최대 1회**만 지급되도록 서버에서 막습니다.
  - 보상이 생기면서 참가비도 기본 100 → **150코인**으로 올렸습니다 (기대 지급액보다 참가비가 높게 유지되도록).
- **"카드 순서 보기"** 버튼으로 코인을 더 내면, 남은 카드 전체 순서를 **본인 화면에만 5초간** 보여주고 패턴을 자유롭게 바꿀 수 있습니다.
  - 패턴 선택 화면뿐 아니라 **게임 중(카드 넘기는 도중)에도** 사용할 수 있습니다.
  - 가격은 고정이 아니라 **남은 카드 수에 따라 자동으로 오릅니다** (기준가 30코인 → 41장 이상 1배, 21~40장 2배, 9~20장 4배, 8장 이하 8배). 서버가 남은 장수로 직접 가격을 다시 계산해서 청구합니다.
...
2. Environment Variables에 `.env.example`의 값을 등록:
   ...
   - `ENTRY_COIN_PRICE` = `150`, `WIN_REWARD_COINS` = `250`, `PEEK_COIN_PRICE_BASE` = `30`
